...
public class INavMeshCarMovement : MonoBehaviour
{
    ...
    private AgentMovement agentObject;
    public Vector3 lastAgentPosition;
    private Queue<Vector3> objectives;
    ...
    void Start()
    {
        ...
        agentObject = agent.GetComponent<AgentMovement>();
        objectives = new Queue<Vector3>();
        currentObjective = agent.transform.position;
        lastAgentPosition = agentObject.lastPosition;
        isStarting = true;
        isBraking = false;
    ...
    private void Steering(WheelCollider lw, WheelCollider rw)
    {
        ...
        else
        {
            ...
            Vector3 relative = transform.InverseTransformPoint(currentObjective);
    ...
    {
        distance = Vector3.Distance(transform.position, currentObjective);
        if (distance < 3f)
        {
            numberOfObjectives--;
            ...
            {
                objectives.Dequeue();
                currentObjective = objectives.Peek();
            } else
            {
                if (objectives.Count > 0) // count == 1
                {
                    objectives.Dequeue();
                }
                currentObjective = lastAgentPosition;
            }
        }
    }
    ...
    {
        if (lastAgentPosition != agentObject.lastPosition)
        {
            lastAgentPosition = agentObject.lastPosition;
            objectives.Enqueue(lastAgentPosition);
            numberOfObjectives++;
        }
    }
